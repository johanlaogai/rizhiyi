...
def update_operation(cursor,filter={}):
    command = 'UPDATE AgentStatus \
            ...
            where id={start_id} && platform="{platform}"'
    script.info(command.format(**filter))
    cursor.execute(command.format(**filter))
    connection.commit()
...
def filter_agent(cursor,iterator,expected_version,platform):
    ...
    iterator = [item for item in iterator if not is_expired(item[1])]
    ...
    for item in iterator:
        ...
        while True:  
            ...
            if num <10: #同时升级的数量不能大于100,
                update_operation(cursor,{"expected_version":expected_version,"start_id":item[5],"platform":platform}) #逐个升级
                i+=1
                break
            script.info("等待30秒")
            sleep(30) #否则等待30秒，直到升级数量小于100
        script.info("等待100秒")
        sleep(100)  #更新后等待100秒


def check_version(cursor,q):  
    while True:  
        sleep(10)
        try:
            item = q.get_nowait()
...
if __name__ == "__main__":
    # q = queue.Queue()
    modle = 'status="Running" && platform="{}" && cur_version="{}" && last_update_timestamp ="{}" && ip!="" limit 300'
    cursor = connection.cursor()
    #检查cur_version和expected_version字段是否相等  
    check_thread = threading.Thread(target=check_version, args=(cursor,q)) 
    ...
    # #升级Linux-x64，当前版本4.4.0.8
    # linux64 = select_operation(cursor,filter=modle.format("linux-x64","4.8.0.7"))
    # filter_agent(cursor,linux64,"4.8.1.2","linux-x64")  #期望版本4.8.0.7
    # #升级win-386，版本3.9.1.5
    win386 = select_operation(cursor,filter=modle.format("win-386","3.6.0.7"))
    # filter_agent(cursor,win386,"3.9.2.0","win-386") #期望版本3.9.2.0
    #升级win-386，版本3.8.0.9
    # win386 = select_operation(cursor,filter=modle.format("win-386","3.8.0.9"))
    # filter_agent(cursor,win386,"3.9.2.0","win-386") #期望版本3.9.2.0
    # script.info("执行结束，等待1800秒")
    # sleep(1800)
    # q.put(None) 
    # q.join()
    check_thread.join()
